    ...
    def move(self):
        if self.dest != "":
            try:
                ...
                print("An Error occured when moving ", self.name, " to destination ", self.dest, "File Already Exists")
            except FileNotFoundError:
                print("An Error occured when moving ", self.name, "The file has been moved or deleted already")
...
class SortImages():
        ...
        for root,dirs,files in os.walk(src,topdown=True):
            ...
            for name in files:
                ...
                if ext == "png" or ext == "gif" or ext == "jpg" or ext == "jpeg" or ext == "bmp" or ext == "pcx" or ext == "tiff" or ext=="webp" or ext=="psd" or ext=="jfif":
                    self.imagelist.append(Imagefile(name, os.path.join(root,name)))
    ...
    def setup(self,src,dest):
        ...
        self.walk(src)
    def getthumb(self,imagefile):
        thumbsize=self.gui.thumbnailsize
        __image = Image.open(imagefile.path)
        MAX_SIZE=(thumbsize,thumbsize)
        thumb=__image.thumbnail(MAX_SIZE)
        thumb.save(os.path.join("data",))
        imagefile.thumbnail=
...
class GUIManager(tk.Tk):
    thumbnailgrid=[3,3]
    thumbnailsize=200
    def __init__(self, fileManager=SortImages) -> None:
        super().__init__()
...
Thanks you for using this program!""")
        ...
        self.imagegridframe = tk.Frame(self.toppane)
        self.imagegridframe['background']='#a9a9a9' # set background color to a darker grey
        self.imagegridframe.grid(row=0,column=1,sticky="NSEW")
        self.toppane.add(self.imagegridframe,weight=3)
